...
2. **AnimeRestore Denoise v2**（`animerestore_plugin.cpp`）：本体。
   render(t) ごとに t±TemporalRadius をホストから取得 → ウィンドウ内で
   保持グループ検出 → グループ解析（キャッシュ再利用）→ 現在フレームを出力。
   v2 の追加：**第2層カット間拡張統合**（Extend Neighbors、1コマ素材にも
   時間統合が効く）＋第3層ブレンド、**ラインノイズ／スキャンノイズ除去トグル**、
   **クリップ端処理**（クリップ範囲外のクランプ複製を要求しない）。
   パラメータ：Mode / Temporal Radius / Dust Removal / Grain Reduction /
   Extend Neighbors / Line Noise Removal / Scan Noise Removal。
   ログ `/tmp/ar_ofx.log`。制約：float RGBA のみ、傷除去は未搭載
   （ショット横断解析が必要）、OpenCV は /opt/homebrew に動的リンク
